...
在[SPDK CI](https://github.com/spdk/spdk-ci/actions)持续化测试网页上，主要有两类自动化测试。其中一类是针对每个改动的测试；还有一类是每晚跑一次的测试。每晚跑一次的测试会跑的更久，涉及更多的测试覆盖。这里我们以每晚跑一次的测试作为例子，来分享一些调试小建议。这些建议同样适用到针对每次改动的测试。

当测试完成后，在[SPDK CI webpage](https://github.com/spdk/spdk-ci/actions)页面上有个`Job results`部分，有多个相关测试结果的链接。点击具体的工作流运行可以查看所有子测试任务和对应的测试结果。我们可以点击失败状态的子任务来具体查看该任务执行的情况，找到相关的`build.log`文件可以查看具体测试失败的原因。拉到网页最下面可以大致获得更多失败信息。
